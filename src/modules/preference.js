...
import { $br, $hr, $progress, $link } from '../components/_elements'
import { Endpoints } from '../constants/endpoints'
import version from '../constants/version'
import { pluginStore } from './pluginStore'
import { _analytics, getSiteID } from './analytics'
...
export const preference = {
   ...
   * @type {<T extends unknown>(setting: string) => T}
   */
  get(setting) {
  ...
  modal() {
    ...
    var $tabContent = $('<div>', {
      ...
      $('<section>', { id: 'plugin' }).append(
        ...
        $('<div>', { class: 'plugin-footer' }).html(
          _msg('preference-plugin-footer', Endpoints.GITHUB_URL)
        )
      ),
        ...
        $('<div>').append(
          ...
          $('<label>', { class: 'choose-skin' }).append(
            ...
            $('<input>', {
              ...
              value: `${Endpoints.PLUGIN_CDN_BASE}/skins/ipe-default.css`,
            })
          )
        ),
      ...
      $('<section>', { id: 'about' }).append(
        ...
        $('<p>').append(
          ...
          ': ',
          $('<a>', {
            href: Endpoints.GITHUB_URL,
            text: Endpoints.GITHUB_URL,
            rel: 'noopener',
            target: '_blank',
          })
        ),
    ...
    ssi_modal.show({
      ...
      onShow($modal) {
        ...
        $.get(`${Endpoints.ANALYTICS_API_BASE}/query/user`, {
          userName,
          siteUrl: getSiteID(),
          prop: '*',
        }).then((ret) => {
          ...
          const dashUrl = `${Endpoints.ANALYTICS_DASH_URL}/user?${$.param({
            userName,
